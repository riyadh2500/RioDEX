...
async function deployContract(wallet, provider, name, artifactPath, constructorArgs = []) {
  ...
  const maxFee  = feeData.maxFeePerGas ?? BigInt('5000000000')
...
async function main() {
  ...
  const updates = {
    NEXT_PUBLIC_FACTORY_BASE_SEPOLIA:       factAddr,
    NEXT_PUBLIC_ROUTER_BASE_SEPOLIA:        routAddr,
    NEXT_PUBLIC_TOKEN_FACTORY_BASE_SEPOLIA: tokAddr,
    NEXT_PUBLIC_WETH_BASE_SEPOLIA:          wethAddr,
  }
